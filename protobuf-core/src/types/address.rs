...
use crate::types::constants::ADDRESS_LENGTH;
...
impl Error {
    fn decode(source: bech32::primitives::decode::CheckedHrpstringError) -> Self {
        Self(ErrorKind::Decode { source })
    }

    fn invalid_prefix(source: bech32::primitives::hrp::Error) -> Self {
        Self(ErrorKind::InvalidPrefix { source })
    }

    fn incorrect_length(received: usize) -> Self {
        Self(ErrorKind::IncorrectLength { received })
    }
}
...
#[cfg(test)]
mod tests {
    use super::{Address, Bech32, Bech32m, Error, ErrorKind};
    ...
    fn assert_wrong_address_bytes(bad_account: &[u8]) {
        ...
        let Error(ErrorKind::IncorrectLength { received }) = error else {
